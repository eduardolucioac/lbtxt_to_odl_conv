...
function Convert_LBTXT()
{
	...
	for(index0 = 0; index0 < arrayOfLines0.length; ++index0)
	{
		...
		if(!blockFields)
		{
			...
			if(arrayOfLines0[index0].indexOf(".DESCRICAO: ") !== -1)
			{
				...
				var fieldDescription = "'" + arrayOfLines0[index0].replace(".DESCRICAO: ", "").split("'")[1] + "'";
				var arrayFieldLine = arrayOfLines0[index0].replace(".DESCRICAO: ", "").replace(fieldDescription + ", ", "").split(", ");
				arrayFieldLine.unshift(fieldDescription);
	...
	for (arrayFieldsIndex0 = 0; arrayFieldsIndex0 < arrayFields.length; arrayFieldsIndex0++)
	{
		if(arrayFields[arrayFieldsIndex0])
		{
									...
									else
									{
										switch(arrayFields[arrayFieldsIndex0][arrayFieldsIndex1][arrayFieldsIndex2])
										{
											...
											case "URL":
												...
												break;
											case "SOM":
												//Note: Não tratado! Implementar! By Questor
												ODLFieldPart0 = "SOM_NOT_IMPLEMENTED ";
												breakForLoop = true;
												lineIterateBreaker++;
												break;
											case "WORD":
												//Note: Não tratado! Implementar! By Questor
												ODLFieldPart0 = "WORD_NOT_IMPLEMENTED ";
												breakForLoop = true;
												lineIterateBreaker++;
												break;
											default:
			...
			fieldNameGroup = "";
			if(ODLFieldPart0 == "SOM_NOT_IMPLEMENTED " || ODLFieldPart0 == "WORD_NOT_IMPLEMENTED ")
			{
				alert("O tipo \"" + ODLFieldPart0.replace("_NOT_IMPLEMENTED ", "") + "\" não está implementado no LBW provider! Esse campo não será mapeado na ODL!");
			}
			else
			{
				...
				else if(isMonoValued == "MULTIVALORADO")
				{
					...
					if(!groupIsAlreadyInTheList)
					{
						...
						if(fieldNameGroup == "")
						{
							arrayMultivaluedGroupsFields[arrayMultivaluedGroupsAddIndex0][0] = ODLFieldPart1_0 + "$NOTGROUPEDMULTIVALUEDFIELD";
	...
	for (arrayMultivaluedGroupsFieldsIndex0 = 0; arrayMultivaluedGroupsFieldsIndex0 < arrayMultivaluedGroupsFields.length; arrayMultivaluedGroupsFieldsIndex0++)
	{
		if(arrayMultivaluedGroupsFields[arrayMultivaluedGroupsFieldsIndex0])
		{
			...
			{
				if(arrayMultivaluedGroupsFields[arrayMultivaluedGroupsFieldsIndex0][0].indexOf(" $NOTGROUPEDMULTIVALUEDFIELD") != -1)
				{
					if(arrayMultivaluedGroupsFieldsIndex1 > 0)
					{
						//Note: Backup! By Questor!
						...
						notGroupedMultivaluedFields = notGroupedMultivaluedFields + "    attribute set<" + newLine + "        struct" + newLine + "        {" + newLine;
						notGroupedMultivaluedFields = notGroupedMultivaluedFields + "            " + arrayMultivaluedGroupsFields[arrayMultivaluedGroupsFieldsIndex0][1] + newLine;
						notGroupedMultivaluedFields = notGroupedMultivaluedFields + "        }" + newLine + "        > " + arrayMultivaluedGroupsFields[arrayMultivaluedGroupsFieldsIndex0][0].replace(" $NOTGROUPEDMULTIVALUEDFIELD", "") + ";" + newLine;
						alert("Para o correto funcionamento do LBW provider faz-se necessário que todo campo multivalorado esteja inserido em um grupo multivalorado. Crie um grupo multivalorado de nome \"" + arrayMultivaluedGroupsFields[arrayMultivaluedGroupsFieldsIndex0][0].replace(" $NOTGROUPEDMULTIVALUEDFIELD", "") + "\" e insira o campo de mesmo nome nesse grupo. A princípio não haverá nenhum efeito colateral ao modo como esses campos são tratados nas aplicações!");
